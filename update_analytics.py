...
export default function ClassAnalytics() {
  ...
  const { data: analyticsData, isLoading } = useClassAnalytics();
  
  const mockClasses = analyticsData?.classes || [];
  const mockStudents = analyticsData?.students || [];
  ...
  const activeClass = mockClasses.find((c: any) => c.name === selectedClass) || mockClasses[0] || null;
  const classStudents = mockStudents.filter((s: any) => s.class === selectedClass || selectedClass === ''); // Assuming first class for now
  const total = classStudents.length || 1;

  // Sorting lists
  const sortedOverall = [...classStudents].sort((a: any, b: any) => b.overallScore - a.overallScore);
  const topThree = sortedOverall.slice(0, 3);
  const criticalList = classStudents.filter((s: any) => s.status === 'Critical');
  const needsImprovementList = classStudents.filter((s: any) => s.status === 'Needs Improvement');
      ...
      >
        ...
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="bg-white px-4 py-2.5 rounded-2xl border border-border/50 shadow-sm flex items-center gap-3">
            ...
            {mockClasses.length > 0 ? mockClasses.map((c: any) => (
              <button
                key={c.id}
                onClick={() => setSelectedClass(c.name)}
                className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors whitespace-nowrap ${
                  selectedClass === c.name || (!selectedClass && c.name === mockClasses[0].name)
              ...
              <motion.div variants={itemVariants} className="bg-white rounded-[32px] p-8 border border-border/40 shadow-sm space-y-6">
                ...
                <div className="space-y-4 text-xs font-bold text-brand-dark">
                  <div className="p-4 bg-muted rounded-2xl space-y-1 border border-border/30">
                    ...
                    <p className="text-sm">{classStudents.sort((a: any,b: any)=>b.attendance-a.attendance)[0]?.name || 'N/A'}</p>
                  </div>
                  <div className="p-4 bg-muted rounded-2xl space-y-1 border border-border/30">
                    <p className="text-muted-foreground uppercase text-xs">Most Consistent Practice</p>
                    <p className="text-sm">{classStudents.sort((a: any,b: any)=>b.streak-a.streak)[0]?.name || 'N/A'}</p>
                  </div>
                  <div className="p-4 bg-muted rounded-2xl space-y-1 border border-border/30">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="text-xs font-bold text-brand-dark divide-y divide-border/20">
                    {classStudents.slice(0, 15).map((std: any) => (
