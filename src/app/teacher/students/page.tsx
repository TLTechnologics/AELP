...
export default function StudentManagement() {
  ...
  // Filter logic
  const filteredStudents = dbStudents.filter((student) => {
    const matchesSearch =
  ...
  const sortedStudents = [...filteredStudents].sort((a, b) => {
    ...
    if (sortBy === 'score') return (a.overallScore - b.overallScore) * multiplier;
    if (sortBy === 'attendance') return (a.attendance - b.attendance) * multiplier;
    return 0;
  });
  ...
  // ── Download all filtered students as CSV ──────────────────────────
  const handleDownloadCSV = () => {
    const rows = sortedStudents.length > 0 ? sortedStudents : dbStudents;
    if (rows.length === 0) return;

    const headers = [
      'Name', 'Email', 'Semester', 'CEFR Level',
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-muted/50 border-b border-border/40 text-xs font-bold text-muted-foreground uppercase tracking-widest">
                  <th onClick={() => toggleSort('name')} className="p-6 cursor-pointer hover:bg-muted/70 transition-colors">
                  ...
                  <th onClick={() => toggleSort('score')} className="p-6 text-center cursor-pointer hover:bg-muted/70 transition-colors">
                  ...
                  <tr>
                    <td colSpan={9} className="p-16 text-center">
                      <div className="max-w-md mx-auto space-y-3">
                        ...
                        <p className="text-sm text-muted-foreground font-medium">The server did not return the directory. Please try again.</p>
                        <button
                          type="button"
                          onClick={() => refetch()}
                    ...
                    <td colSpan={9} className="p-16 text-center">
                      <div className="max-w-md mx-auto space-y-3">
                        ...
                        <p className="text-sm text-muted-foreground font-medium">
                          {dbStudents.length === 0
                            ? 'No students have been added yet. Use the Add Student tab to get started.'
