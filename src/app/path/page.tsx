...
import { Check, Lock, Play, Star, AlertCircle, TrendingUp, Map } from 'lucide-react';
      ...
      >
        ...
        {recommendations.length === 0 ? (
          ...
          <motion.div variants={itemVariants} className="relative flex flex-col items-center mt-16">
            {/* Connecting Line */}
            <div className="absolute top-10 bottom-10 w-4 bg-muted/40 rounded-full z-0 border-x border-border/50" />
            <motion.div 
              initial={{ height: 0 }}
              animate={{ height: '20%' }}
            ...
            {recommendations.map((node: any, i: number) => {
              const isLeft = i % 2 !== 0;
              const offsetClass = isLeft ? '-translate-x-20 sm:-translate-x-24' : 'translate-x-20 sm:translate-x-24';
              ...
              return (
                <motion.div 
                  ...
                  className={`relative z-10 w-full flex flex-col items-center mb-20 ${offsetClass}`}
                >
                  {/* Tooltip Label */}
                  <div className={`mb-6 w-[280px] sm:w-[320px] bg-white p-6 rounded-[24px] shadow-sm border border-border/80 text-sm flex flex-col items-center transition-all hover:-translate-y-2 hover:shadow-lg cursor-pointer ${isLocked ? 'opacity-80 hover:opacity-100' : 'hover-card-up'}`}>
                  ...
                  <button 
                    ...
                    className={`w-24 h-24 rounded-[32px] flex items-center justify-center shadow-sm transition-all duration-300 relative group 
                      ...
                      ${isLocked ? 'bg-muted/40 border-4 border-border/50 text-muted-foreground cursor-not-allowed' : ''}
                    `}
                  >
                    ...
                    {isLocked && <Lock className="w-8 h-8 opacity-50" strokeWidth={3} />}
